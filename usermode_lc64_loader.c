...
void enter_lc64(void * entry, long argc, char ** argv,
                char ** envp, void * stack_top);
...
static off_t get_file_size(int fd) {
  struct stat st;  return fstat(fd, &st) < 0 ? -1 : st.st_size;
}
...
static void * x_mmap_fixed_rwxo(void * addr, size_t len) {
  ...
  if (p == MAP_FAILED && errno == EEXIST) p = MAP_FAILED;
  if (p != MAP_FAILED) return p;
#endif
  void * p2 = mmap(addr, len, prot,
...
int main(int argc, char * argv[]) {
  ...
  if (region == MAP_FAILED)
    die("mmap at 0x%lx failed: %s", (unsigned long)base, strerror(errno));
  uint8_t * dst = region;
  ...
  sp &= ~((uintptr_t) 0xF);
  enter_lc64((void *) base, guest_argc, guest_argv, guest_envp, (void *) sp);
  _exit(111);
}
